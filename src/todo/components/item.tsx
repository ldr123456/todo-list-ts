...
export const Item: React.FC<ItemProps> = ({ todo, dispatch }) => {
    ...
    const updateItem = useCallback((newTitle: string) => dispatch({ type: UPDATE_ITEM, payload: { id, title: newTitle } }), [dispatch, id]);
    ...
    return (
        <li className={classnames({ completed: todo.completed })} data-testid="todo-item">
            <div className="view">
                {isWritable ? (
                    <Input placeholder="123" onSubmit={handleUpdate} label="Edit Todo Input" defaultValue={title} onBlur={handleBlur} />
                ) : (
                    <>
